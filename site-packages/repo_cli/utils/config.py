...
def load_token(site):
    ...
    url = config.get('url')
    if not url:
        logger.debug("Repository URL is not set. Please configure. Skipping token check")
        return

    for token_dir in TOKEN_DIRS:
        logger.debug("[LOAD TOKEN] Looking for token in {token_dir}".format(token_dir=token_dir))
        tokenfile = join(token_dir, '%s.token' % quote_plus(url))
        ...
        if _is_file:
            ...
            logger.debug("[LOAD TOKEN] Token file {tokenfile} not found.".format(tokenfile=tokenfile))
