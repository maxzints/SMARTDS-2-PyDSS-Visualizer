...
INITIAL_SPACE = '     '
fmt_package_headers = INITIAL_SPACE + '%(channel_path)-30s | %(name)-40s | %(version)8s | %(family)-12s ' \
                                       '| %(build_number)-10s | %(license)-15s | %(subdirs)-15s'

fmt_package_simple_headers = INITIAL_SPACE + '%(name)-40s | %(file_count)-12s | %(download_count)-12s ' \
                                       '| %(license)-15s'
fmt_channel_headers = INITIAL_SPACE + '%(channel_path)-30s | %(privacy)-10s | %(owners)15s | %(artifact_count)-12s ' \
...
class MirrorFormatter:
            ...
            if key == 'filters':
                ...
                if value:
                    for filter_key in sorted(value.keys()):
                        resp.append("%s   %s| %s" % (INITIAL_SPACE, filter_key.ljust(25), value[filter_key]))
            else:
    ...
    def format_list_item(mirror):
        mirror = {key.replace('mirror_', ''): (val if val is not None else '') for key, val in mirror.items()}
...
class CVEFormatter(FormatterBase):
    ...
    fmt_headers = INITIAL_SPACE + '%(id)-14s | %(score)6s | %(score_type)10s | %(curated)8s | %(packages_count)10s | %(description)-50s'
    ...
    fields = [
        'id', 'curated', 'score', 'score_type', 'description',
...
class CVEFilesFormatter(FormatterBase):
    ...
    fmt_headers = INITIAL_SPACE + '%(channel)-40s | %(artifact_family)-10s | %(cve_status)-10s | %(common_name)-18s | %(ckey)-32s'
    ...
    def normalize_item(item):
        ...
        item_['channel'] = (
            ...
            if item_['parent_channel_name'] else item_['channel_name']
        )
        return item_
...
class PackageFilesFormatterWithCVE(FormatterBase):
    ...
    fmt_headers = INITIAL_SPACE + '%(ckey)-60s | %(cve_score)-9s | %(cve_status)-10s | %(version)-10s | %(platform)-10s'
...
class SettingsFormatter(FormatterBase):
    ...
    def format_object_as_list(cls, obj):
        return cls.format_list([
            {"key": k, "value": v} for k, v in obj.items()
        ])
