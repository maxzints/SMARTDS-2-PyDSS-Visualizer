...
from ..utils.config import get_config, PACKAGE_TYPES, DEFAULT_CONFIG
...
def upload_file(base_url, token, filepath, channel, verify_ssl):
    ...
    response = requests.post(url, files=multipart_form_data, headers={'X-Auth': f'{token}'}, verify=verify_ssl)
    return response
...
class SubCommand(SubCommandBase):
    ...
    def main(self):
        ...
        if not channels:
            ...
            if not channels[0]:
                raise errors.NoDefaultChannel("User default channel is not specified. Please set it in the user account or use -c option.")
    ...
    def add_parser(self, subparsers):
            description = 'Upload packages to your Anaconda Team Edition repository'
            ...
            parser.add_argument('-c', '--channel', action='append', default=[], metavar='CHANNELS', dest='channels')
