...
from anaconda_navigator.config import CONF_PATH, LAUNCH_SCRIPTS_PATH, WIN
from anaconda_navigator.utils.programs import is_program_installed
from anaconda_navigator.utils.win_elevate import run_as_admin

if typing.TYPE_CHECKING:
    import typing_extensions

    class CommandDetails(typing_extensions.TypedDict):
    ...
    class GetCommand(typing_extensions.Protocol):  # pylint: disable=too-few-public-methods
...
DEVNULL: 'typing_extensions.Final[str]' = 'nul' if (sys.platform == 'win32') else '/dev/null'
LOG_FILE: 'typing_extensions.Final[str]' = '{package}-{level}-{index}.txt'
...
def to_posix_path(value: str) -> str:
    """Convert path to a posix-compatible."""
    return value.replace(os.path.sep, posixpath.sep)


def safe_unix(value: str) -> str:
    """Prepare argument which is safe to use in unix command line."""
    to_replace: 'typing_extensions.Final[typing.Set[str]]' = {'"', '$', '\\', '`'}
    to_escape: 'typing_extensions.Final[typing.Set[str]]' = {' ', '&', ';', '<', '>', '|'} | to_replace
...
def safe_windows(value: str) -> str:
    ...
    to_escape: 'typing_extensions.Final[typing.Set[str]]' = {' ', '%', '"', '&', ';', '<', '>', '|'}
...
def safe_argument(value: str) -> str:
    """Prepare argument which is safe to use in command line."""
    if WIN:
        return safe_windows(value)
    return safe_unix(value)
...
class RunningProcess:
    ...
    def __init__(
        ...
        self.__package: 'typing_extensions.Final[str]' = package
        self.__process: 'typing_extensions.Final[subprocess.Popen]' = process
        self.__return_code: typing.Optional[int] = None
        self.__stdout: 'typing_extensions.Final[typing.Optional[str]]' = stdout
        self.__stderr: 'typing_extensions.Final[typing.Optional[str]]' = stderr
        self.__birth: 'typing_extensions.Final[datetime.datetime]' = datetime.datetime.utcnow()
...
def get_command_on_win(  # pylint: disable=too-many-arguments
    ...
    command = dict((
        ...
        (
            str(
                'start powershell.exe -ExecutionPolicy ByPass -NoExit -Command  '
    ...
    return {  # popen_dict
        'creationflags': 0x0800200,  # Create no window, Create new process group
        'shell': True,
        'cwd': cwd,
        'env': environment,
    }, {
    ...
    if WIN:
        ...
        if as_admin:
            if run_as_admin(args):
                return typing.cast(typing.Any, True)
            return None
...
def console(activate=None, working_directory=os.path.expanduser('~'), term_command=''):
    ...
    if os.name == 'nt':
        if activate:
            ...
            cmd = f'start cmd.exe /k "activate "{activate}" & {term_command}"'
        else:
            cmd = 'start cmd.exe'
        # pylint: disable=consider-using-with
        subprocess.Popen(cmd, shell=True, cwd=cwd)  # nosec
    elif sys.platform == 'darwin':
        if activate:
            from anaconda_navigator.api.conda_api import CONDA_API  # pylint: disable=import-outside-toplevel
            rootprefix = CONDA_API.ROOT_PREFIX
            cmd = f'''\
#!/usr/bin/osascript
tell application "Terminal"
    activate
    do script ". {rootprefix}/bin/activate && conda activate {activate}; {term_command}"
end tell
'''
        else:
            cmd = 'bash'
        ...
        with open(fname, 'w') as f:  # pylint: disable=invalid-name,unspecified-encoding
            f.write(cmd)
        os.chmod(fname, 0o755)  # nosec

        subprocess.call([fname], shell=True, cwd=cwd)  # nosec
        ...
        if is_program_installed('gnome-terminal'):
            ...
            # pylint: disable=consider-using-with
            subprocess.Popen(cmd, cwd=cwd)  # nosec
...
def check_prog(prog: str, prefix: typing.Optional[str] = None) -> bool:
    ...
    return any(api.conda_package_version(prefix=prefix, pkg=p) is not None for p in pkgs)
...
def py_in_console(activate=None, prog='python'):
    ...
    if not check_prog(prog, activate):
        raise RuntimeError(f'Program not available in environment: {prog}, {activate}')
    if prog == 'python':
        cmd = 'python -i'
    elif prog == 'ipython':
        cmd = 'ipython -i'
