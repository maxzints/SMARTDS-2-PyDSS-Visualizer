...
from qtpy.QtWidgets import QDesktopWidget  # pylint: disable=no-name-in-module
try:
...
class _GATracker:
    ...
    TRACKER_ID = 'UA-27761864-8'  # pylint: disable=invalid-name
    DEFAULT_TIMEOUT = 5  # pylint: disable=invalid-name
    ...
    def setup(self):
        ...
        hashed_user = hashlib.md5(ensure_binary(normalized_user)).hexdigest()  # nosec
        ...
        operating_system = ';'.join([platform.uname()[0]] + list(platform.uname()[2:]), )
        pyqt_version = (
            pkg_version(pkg='pyqt5', name='root') or pkg_version(pkg='pyqt4', name='root')
        ...
        self.track_event('application', 'screen-resolution', label=self.get_screen_resolution())
        self.track_event('application', 'operating-system', label=operating_system)
        self.track_event('application', 'version', label=app_version)
        self.track_event('application', 'platform', label=self.api.conda_platform())
        self.info = {
    ...
    def _clean(self):
        ...
        if self._threads:
            for t in self._threads:  # pylint: disable=invalid-name
                if t.isFinished():
    ...
    def set_client_id(self, client_id=None):
        ...
        self.tracker = Tracker.create(  # pylint: disable=attribute-defined-outside-init
    ...
    def get_screen_resolution():
        ...
        try:
            ...
            value = f'{geometry.width()}x{geometry.height()}'
        except Exception:  # pylint: disable=broad-except
            value = None
        return value
...
def GATracker():  # pylint: disable=invalid-name
    """Google Analytics Tracker."""
    global GA_TRACKER  # pylint: disable=global-statement
...
# --- Local testing
# -----------------------------------------------------------------------------
def print_test(worker, output, error):  # pragma: no cover ; pylint: disable=unused-argument
