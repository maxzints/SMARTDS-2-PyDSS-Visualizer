...
def generate_uuid(basedata=None):
    ...
    if basedata is None:
        ...
        checksum = hashlib.md5(basedata).hexdigest()  # nosec
...
class Tracker(object):
    ...
    option_sequence = {
        ...
        'timing':
        [(six.string_types, 'utc'), (six.string_types, 'utv'), (six.string_types, 'utt'), (six.string_types, 'utl')],
    }
    params = None
    parameter_alias = {}
    ...
    def __init__(
        self, account, name=None, client_id=None, hash_client_id=False, user_id=None, user_agent=None, use_post=True
    ):
        """Primary tracking interface for Universal Analytics.
...
Tracker.alias(
    safe_unicode, 'cu', 'currency', 'transactionCurrency', 'transaction-currency'
...
for product_index in range(1, MAX_EC_PRODUCTS):
    ...
    for list_index in range(1, MAX_EC_LISTS):
        ...
        Tracker.alias(six.text_type, 'il{0}pi{1}nm'.format(list_index, product_index))
        # Product impression brand
        Tracker.alias(six.text_type, 'il{0}pi{1}br'.format(list_index, product_index))
        # Product impression category
        Tracker.alias(six.text_type, 'il{0}pi{1}ca'.format(list_index, product_index))
        # Product impression variant
        Tracker.alias(six.text_type, 'il{0}pi{1}va'.format(list_index, product_index))
