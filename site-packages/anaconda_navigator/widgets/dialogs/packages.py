...
class PackagesDialog(DialogBase):  # pylint: disable=too-many-instance-attributes
    ...
    def setup(self, worker, output, error):  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
        ...
        if error:
            ...
            if self.api.is_offline():
                error = str(
                    ...
                    'Installation and upgrade actions will be subject to the packages currently available on your '
                    'package cache.'
                )
            ...
            actions_link_names = {p['name'] for p in actions_link}
            actions_unlink_names = {p['name'] for p in actions_unlink}
        ...
        self.table.resizeColumnsToContents()
        width = sum(self.table.columnWidth(i) for i in range(self.table.columnCount())) + 10
