...
class BackupSelectorDialog(multiaccount_dialogs.PrepopulatedSelectorDialog):  # pylint: disable=too-few-public-methods
    ...
    def __check_nucleus_inject(self, result: nucleus.TaskResult) -> None:
        ...
        if result.status == nucleus.TaskStatus.SUCCEEDED:
            new_worker: nucleus.TaskWorker
            new_worker = nucleus.NucleusAPI().create_environment.worker(  # pylint: disable=no-member
    ...
    def _handle_environment_already_exists(
            self,
            path: str,
    ) -> 'nucleus_error_parsers.Handler[requests.RequestException]':
        """
        ...
        def result(exception: BaseException) -> bool:  # pylint: disable=unused-argument
            if self.environment_override:
                worker: nucleus.TaskWorker  # pylint: disable=no-member
                worker = nucleus.NucleusAPI().update_environment.worker(  # pylint: disable=no-member
                    name=self.environment_name,
                    path=path,
                )
    ...
    def _handle_unprocessable_entry(self) -> 'nucleus_error_parsers.Handler[requests.RequestException]':
