...
if typing.TYPE_CHECKING:
    ...
    class ComponentInitializer(typing_extensions.Protocol):  # pylint: disable=too-few-public-methods
...
class MainWindow(QMainWindow):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-branches,too-many-locals,too-many-statements
        ...
        for _ in anaconda_solvers.POOL.solve():
            pass
        ...
        solved_error: issue_solvers.SolvedError
        issue_context: issue_solvers.ConfigurationContext = issue_solvers.ConfigurationContext(
            api=self.api,
            config=self.config,
        )
        for solved_error in issue_solvers.CONFIGURATION_POOL.solve(context=issue_context):
            self.components.notifications.information_message_box(
                text=solved_error.message,
                title=solved_error.caption,
            )
        ...
        self._timer_health_check: 'typing_extensions.Final[QTimer]' = QTimer()
        ...
        if youtube_url:
            self.stack.add_social('Youtube', url=youtube_url)
        if github_url:
            self.stack.add_social('Github', url=github_url)
    ...
    def setup(self, worker, output, error):
        ...
        """
        solved_error: issue_solvers.SolvedError
        solved_tags: typing.Set[str] = set()
        issue_context: issue_solvers.ConflictContext = issue_solvers.ConflictContext(
            api=self.api,
            config=self.config,
            conda_info=output,
        )
        for solved_error in issue_solvers.CONFLICT_POOL.solve(context=issue_context):
            self.components.notifications.information_message_box(
                text=solved_error.message,
                title=solved_error.caption,
            )
            solved_tags.update(solved_error.tags)
        if 'default_env' in solved_tags:
    ...
    def post_setup(self, conda_data):
        ...
        if geo:
            ...
            try:
                self.restoreGeometry(pickle.loads(geo))  # nosec
                self.show()
    ...
    def show_error_message(self, error_name, error_text):
        ...
        if self.api.is_offline():
            ...
            if 'PackagesNotFoundError' in error_name:
                ...
                error_msg = str(
                    'Some of the functionality of Anaconda Navigator will be limited in <b>offline mode</b>. <br><br>'
                    'Installation and upgrade of packages will be subject to the packages currently available on '
                    'your package cache.'
                )
    ...
    def show_channels(self, button=None, sender=None):
        ...
        brand = self.api.config.get('main', 'logged_brand')
    ...
    def check_package_cache(self) -> None:
        ...
        # pylint: disable=protected-access,unpacking-non-sequence
    ...
    def set_busy_status(self, conda=None, learning=None, community=None):
        """
        ...
        if not any([self.busy_conda, self.busy_learning, self.busy_community]):
            self.sig_ready.emit()
