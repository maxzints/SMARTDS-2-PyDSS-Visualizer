...
from anaconda_navigator.utils.notifications import NOTIFICATIONS_QUEUE
...
@functools.lru_cache()
def construct_message_box(text: str = '', title: str = '') -> 'QtWidgets.QDialog':
    """Initialize new :class:`~dialogs.MessageBoxInformation` instance."""
    from anaconda_navigator.widgets import dialogs  # pylint: disable=import-outside-toplevel

    dialog: dialogs.MessageBoxInformation = dialogs.MessageBoxInformation(text=text, title=title, analytics=False)
    dialog.setMinimumWidth(480)
    return dialog


@contextlib.contextmanager
def catch_and_notify(once: bool = True) -> typing.Iterator[None]:
    """
    ...
    """
    def add_to_queue(text: str = '', title: str = '') -> None:
        """Create and add dialog to the queue."""
        msg_box: typing.Optional['QtWidgets.QDialog'] = construct_message_box(text, title)

        if msg_box is None or (once and msg_box in NOTIFICATIONS_QUEUE.queue):
            return

        NOTIFICATIONS_QUEUE.add(msg_box)

    try:
        warns: typing.List[warnings.WarningMessage]
        ...
        for warning in warns:
            add_to_queue(title=warning.category.__name__ if warning.category else 'Warning',
                         text=str(warning.message))  # pylint: disable=no-member

    except binstar_client.BinstarError as error:
        add_to_queue(title=error.__class__.__name__, text=str(error.message))
